    ...
    $.ajax({
        ...
        success : function (response) {
            ...
            for(let i = 0; i < response.length ; i++){
                ...
                newText = document.createTextNode(response[i]['name'] + " " + response[i]['last_name']);
                newCell.appendChild(newText);
                newCell.style.textAlign = 'center';

                newCell = newRow.insertCell(2);
                newText = document.createTextNode(response[i]['dni']);
                newCell.appendChild(newText);
                newCell.style.textAlign = 'center';

                if (response[i]['type'] === 'l' ){
                    response[i]['type'] = 'Registrado';
                }
                else{
                    response[i]['type'] = 'No Registrado';
                }

                newCell = newRow.insertCell(3);
                newText = document.createTextNode(response[i]['type']);
                newCell.appendChild(newText);
                newCell.style.textAlign = 'center';
